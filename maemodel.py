...
class MAE_MODEL(nn.Module):
    ...
    def forward(self, imgs, mask_ratio=0.75):
        ...
        # print('forward_encoder:', time.time() - tic)
        # tic = time.time()

        pred = self.forward_decoder(latent, ids_restore)  # [N, L, p*p*1]

        # print('forward_decoder:', time.time() - tic)
        # tic = time.time()

        loss = self.forward_loss(imgs, pred, mask)

        # print('forward_loss:', time.time() - tic)

        # return loss, pred, mask
        return latent[:, 0]


    def forward_shuffled(self, imgs, mask_ratio=0.75):
        # import time
        # tic = time.time()
        ...
        # Generate a random shuffle index except the CLS token
        x_to_shuffle = x[:, 1:, :]
        total_seq_length = x_to_shuffle.shape[1]
        # Shuffle all tokens except CLS
        ids_shuffle = torch.randperm(total_seq_length).repeat(x_to_shuffle.shape[0], 1).to(x_to_shuffle.device)
        x_shuffled = torch.gather(x_to_shuffle, dim=1,
                                  index=ids_shuffle.unsqueeze(-1).repeat(1, 1, x_to_shuffle.shape[2]))
        # Shuffle mask similarly to maintain consistency
        mask_shuffled = torch.gather(mask, dim=1, index=ids_shuffle)
        x_shuffled = torch.cat([x[:, :1, :], x_shuffled], dim=1)  # append cls token
        x = x_shuffled
        # apply Transformer blocks
        for blk in self.decoder_blocks:
            x = blk(x)
